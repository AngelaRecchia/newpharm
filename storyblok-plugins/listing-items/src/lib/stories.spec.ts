...
describe('searchStories', () => {
  ...
  it('per catalogo carica downloadable kind=catalog', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = requestUrl(input)
      if (url.includes('/spaces/me')) {
        return jsonResponse({ space: { version: 1 } })
    ...
    expect(result.map((story) => story.uuid)).toEqual(['dl-1', 'same'])
    expect(result[0].name).toBe('Catalogo nuovo')

    const storyCalls = storyRequestUrls(fetchMock)
    expect(storyCalls).toHaveLength(1)
    expect(storyCalls[0].searchParams.get('content_type')).toBe('downloadable')
    expect(storyCalls[0].searchParams.get('filter_query[kind][in]')).toBe('catalog')
  })
})
