 ...
 * basandosi su VERCEL_ENV o NODE_ENV
 */
export function isProduction(): boolean {
  ...
  // Se VERCEL_ENV non è impostata, usa NODE_ENV come fallback
...
export async function getCacheVersion(): Promise<number | undefined> {
  ...
  try {
    ...
    const cv = data?.space?.cache_version || data?.space?.cv || data?.cache_version || data?.cv
